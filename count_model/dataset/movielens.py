...
from count_model.assessor.nb_factor import NBFactor
import numpy
...
def make_user_item_interaction(row) -> Interaction:
    return Interaction(
        subject=get_user_id(row),
        verb=get_action_id(row),
        object=get_item_id(row),
    )


def make_iterable_user_item_interaction(row) -> Iterable[Interaction]:
    return (
        Interaction(
            subject=get_user_id(row),
            verb=get_action_id(row),
...
def get_user_item_interactions(df):
...
def compute_evaluations_2(
    train_df,
    test_df,
    assess_function,
    evaluation_functions,
    # action_factories: List[Callable[[pandas.DataFrame], Iterator[Interaction]]],
):
    action_indicies = []
    ...
    for user_id, test_actions in test_df.groupby("user_id"):
        ...
        for idx, action_row in test_actions.iterrows():
            conditioning_user_action_df = train_user_actions[
                train_user_actions["timestamp"] < action_row["timestamp"]
            ]
            # conditioning_interactions = []
            # for idx, action_row in conditioning_user_action_df.iterrows():
            #     for action_factory in action_factories:
            #         conditioning_interactions.extend(action_factory(action_row))

            # conditioning_actions = get_conditioning_actions(action_row)
            # test_interaction = make_user_item_interaction(action_row)
            assesment = assess_function(
                conditioning_user_action_df,
                action_row,
            )
    ...
    res = test_df.loc[action_indicies].copy()
...
def compute_scores_2(
    ...
    evaluations = compute_evaluations_2(df, test_df, assessment_function, evaluation_functions)
    
    def summarize_evaluations(evals):
        result = []
        for name, _ in evaluation_functions:
            e = evals[name]
            result.append(
                Evaluation(
                    name,
...
def compute_evaluations(
    ...
    for user_id, test_actions in test_df.groupby("user_id"):
        ...
        for idx, action_row in test_actions.iterrows():
            ...
            # conditioning_actions = get_conditioning_actions(action_row)
            action = make_user_item_interaction(action_row)
            assesment = assess_function(
...
def compute_ndcg_for_counter(
    ...
    for user_id, test_actions in test_df.groupby("user_id"):
        conditioning_actions = list(
            get_user_item_interactions(train_df[train_df["user_id"] == user_id])
        )
        user_action_seq = [
            (
                assess_function(
...
def compute_scores(
    ...
    train_df,
    test_df,
    assessment_function,  # could be BinaryAssessor
):
    scores = (
        ("log", log_score),
        ("brier", brier_score),
